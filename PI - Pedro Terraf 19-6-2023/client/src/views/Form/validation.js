let Validate = (activity) => {
  ...
  if (!activity.duration) {
    ...
    //NO PUEDE SER DIFERENTE A UN NUMERO QUE ESTE ENTRE EL 1 Y EL 24 INCLUIDOS
    error.duration =
      "The duration of the activity can only be numbers between 1 and 24, inclusive";
  } else if (activity.duration < 1 && activity.duration > 24) {
  ...
  if (!activity.season) {
    ...
    activity.season === "Otoño" ||
    activity.season === "Verano" ||
    activity.season === "Invierno" ||
    activity.season === "Primavera" ||
    activity.season === "Autumn" ||
    activity.season === "Summer" ||
    activity.season === "Winter" ||
    activity.season === "Spring"
  ) {
    error.season = "The activities season does not exist";
  } else error.season = "";
